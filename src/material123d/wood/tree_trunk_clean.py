...
class TreeTrunk:
        ...
        for n in range(num_rings):
            ...
            with BuildPart() as _model:
                ...
                with BuildSketch(Plane(origin=points[0], z_dir=z_dir)) as ring_profile:
                    # For now, use simple circles to ensure the spacing works correctly
                    # We can add back the natural variations later once the basic structure is confirmed
                    Circle(r2, mode=Mode.ADD)
                    Circle(r1, mode=Mode.SUBTRACT)
    ...
    def export(self):
        ...
        if P.do_export:
            ...
            # Export all rings as separate files
            for i, ring in enumerate(self.part):
                ring_path = P.export_folder / f"tree_trunk_ring_{i+1}.stl"
                ring.export_stl(str(ring_path))
                debug(f"Exported ring {i+1} to {ring_path}")
        else:
